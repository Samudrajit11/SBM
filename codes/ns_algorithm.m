function result=ns_algorithm(obs,model,misc,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   If step_mod = 0, then the evolver routine should initiate the variable by itself.
%
...
if ~isfield(options,'nwalkers')
  if isfield(misc,'hasty') && misc.hasty
    ...
    options.nwalkers=200;
  end
end
if ~isfield(options,'stoprat')
  options.stoprat=10^(-3); % Z-ratio below which to stop nested sampling
end
if ~isfield(options,'ntest')
  options.ntest=500;
end
if isfield(options,'evolver')
  evolver=options.evolver;
else
%  evolver=@(varargin) dhmc_evolve(varargin{:},struct('nsteps',5));
  evolver=@(varargin) cube_evolve(varargin{:},struct('nsteps',5));
end
if isfield(options,'tuner')
  tuner=options.tuner;
else
  tuner=@(step_mods) ns_step_mod_mean(step_mods);
end

if isfield(options,'maxsamples')
  maxsampm1=abs(options.maxsamples)-1;
else
  maxsampm1=Inf;
end
...
step_mod = []; 		%Tell the ns_evolve routine to initialize step_mod 
...
while (Zrat>options.stoprat) 	%Stops when the increments of the integral are small
    ...
    if length(walkers_in_reserve)<nremove
      ...
      for ipar=1:max(Nparfor,nremove)
        ...
        if isfield(model,'recombination')
          ...
          [walker_new(ipar),new_step_mod{ipar}]=evolver(obs,model,logLstar,walker_new(ipar),step_mod,misc);
        else
          %Evolve copied walker within constraint
          [walker_new(ipar),new_step_mod{ipar}]=evolver(obs,model,logLstar,walkers(copy(ipar)),step_mod,misc);
        end
        if isequal(walker_new(ipar).u,walkers(copy(ipar)).u)
          fprintf('Warning: an evolved copy matched the original around iteration %i\n',i);
        end
      end
      if length(new_step_mod)>1
        step_mod=tuner(new_step_mod);
      else
    ...
    if mod(i,ceil(options.ntest/nremove)) == 0
      fprintf('After %i processed samples: Zrat = %.3g\n',i*nremove,Zrat);
      if isfield(options,'test')
        testlist(i/options.ntest).res=options.test(obs,model,logLstar,walkers,step_mod);
      end
    end
