...
%   Similarly for in_model.x2u and u2x
% 
...
if isfield(in_model,'x2u')
  out_model.x2u=@(obs,theta) x2u_func(obs,theta,in_model.x2u,base,[]);
end
if isfield(in_model,'u2x')
  out_model.u2x=@(obs,theta) x2u_func(obs,theta,in_model.u2x,base,[]);
end
if isfield(in_model,'grad_x')
  out_model.grad_x=@(obs,theta) x2u_func(obs,theta,in_model.grad_x,base,{});
end
if isfield(in_model,'prior_disc')
  out_model.prior_disc=@(disc) prod(cellfun(in_model.prior_disc,disc));
end
%if isfield(in_modelt,'slice_head')
%  out_model.slice_head=in_model.slice_head;
%end
...
%  end
%end
if isfield(base,'evolve_extra') || isfield(in_model,'evolve_extra')
  out_model.evolve_extra=@(obs,model,logLstar,walker,step_mod,varargin) evolve_func(obs,model,logLstar,walker,step_mod,in_model,base);
end
...
function [walker,step_mod] = evolve_func(obs,model,logLstar,walker,step_mod,in_model,base)
  if isequal(step_mod,[])
    step_mod={0};
  end
  ispsi=isfield(base,'evolve_extra');
  if ispsi
    [walker,step_mod{1}]=base.evolve_extra(obs,model,logLstar,walker,step_mod{1});
  end
  theta_0=walker.theta{1};
  ts=base.ts(theta_0);
  isex=isfield(in_model,'evolve_extra');
  ...
  if isex
    ...
    for i=1:size(ts,1)
      ...
      [walker_i,step_mod{i+ispsi}] = in_model.evolve_extra(obs(ts(i,1):min(ts(i,2),end),:),model_mod,logLstar,walker_i,step_mod{i+ispsi},step_mod{i+ispsi},mode);
