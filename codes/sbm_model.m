...
if isequal(t0_invprior,0)
  ...
  model.genu=@(obs) struct('cr_sigma1',rand(1,1),'cr_alpha',rand(1,1));
  model.adjust_u=@(u,obs) adjust_u_func(u,2);
  model.invprior=@(u,obs) [sigma1_invprior(u.cr_sigma1(1)) alpha_invprior(u.cr_alpha(1))];
  model.logl=@(obs,theta) sbm_main(obs,0,theta(1),theta(2),0,'l');
  model.x2u=@(obs,theta) sbm_main(obs,0,theta(1),theta(2),0,'u');
  model.u2x=@(u,theta) sbm_main(u,0,theta(1),theta(2),0,'x');
  ...
  model.x2u=@(obs,theta) sbm_main(obs,0,theta(1),theta(2),theta(3),'u');
  model.u2x=@(u,theta) sbm_main(u,0,theta(1),theta(2),theta(3),'x');
